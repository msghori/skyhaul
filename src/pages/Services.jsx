...
export default function Services() {
  ...
  return (
    <>
      <Helmet>
        <title>Our Services - Skyhaul Logistics | Road, Air, Sea Freight & More</title>
        <meta name="description" content="Explore Skyhaul's comprehensive logistics services including road freight, air freight, sea freight, warehousing, customs clearance, and supply chain management." />
      </Helmet>
      ...
      <section className="relative bg-skyblue pt-32 pb-20 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl sm:text-5xl font-extrabold text-white mb-4 animate-fade-in-up">Our Services</h1>
          <p className="text-white/80 text-lg max-w-2xl mx-auto animate-fade-in-up animation-delay-200">
            Comprehensive logistics solutions designed to move your business forward with efficiency, reliability, and care.
          </p>
        </div>
      </section>
  ...
  return (
    ...
    >
      <div className="w-14 h-14 rounded-xl bg-skyblue group-hover:bg-gold-400/10 flex items-center justify-center mb-5 transition-colors duration-300">
        <Icon size={28} className="text-white group-hover:text-gold-500 transition-colors duration-300" />
      </div>
...
function ProcessSection() {
  const [ref, isInView] = useInView()
  const steps = [
    { num: '01', title: 'Consultation', text: 'We assess your logistics requirements and recommend the optimal solution.' },
    { num: '02', title: 'Planning', text: 'Our team creates a detailed logistics plan tailored to your timeline and budget.' },
    { num: '03', title: 'Execution', text: 'We manage every aspect of your shipment from collection to final delivery.' },
    { num: '04', title: 'Support', text: 'Real-time tracking and dedicated support throughout the entire journey.' },
  ]
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {steps.map((step, i) => (
            ...
            >
              <div className="text-4xl font-extrabold text-navy-900/10 mb-2">{step.num}</div>
              <h3 className="font-bold text-navy-900 mb-2">{step.title}</h3>
              <p className="text-navy-500 text-sm leading-relaxed">{step.text}</p>
            </div>
          ))}
        </div>
      </div>
